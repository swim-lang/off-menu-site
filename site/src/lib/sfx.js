...
// Pencil scribble — the real recorded pencil, cropped to just the first
// little scribble stroke (see public/sounds/pencil.wav).
export function playPencil() {
  try {
    const ac = audioCtx()
    if (!ac) return
    loadSample('sounds/pencil.wav').then((s) => {
      if (!s) return
      const now = ac.currentTime
      const src = ac.createBufferSource()
      src.buffer = s.buffer
